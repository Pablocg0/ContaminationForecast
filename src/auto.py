...
from NNSystem.neuralNetworKeras import train as nngk
...
def trainNeuralNetworksKeras(est, dirr, dirTrain, fechaFinal, contaminant, iteraciones):
    """
    Function to train the neuralNetwork of the 23 stations,
    save the training on file trainData/[nameStation].csv
    ...
    while i <= tam:
        ...
        if os.path.exists(newD + '.csv'):
            ...
            build = build.fillna(value=-1)
            xy_values = an(data, build, contaminant)  # preprocessing
            nngk(xy_values[0], xy_values[1], xy_values[2], iteraciones, station, contaminant, dirTrain)  # The neural network is trained
            i += 1
        else:
            i += 1
