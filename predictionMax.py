...
def prediction(station,contaminant,arrayPred,maxx):
    """
    ...
    loss= tf.reduce_mean(tf.abs(y_target - (final_output*maxx)));
    ...
    with tf.Session() as sess:
        saver.restore(sess, 'trainData/noNorm/'+station+'/'+name+'.ckpt'); #we load the training
